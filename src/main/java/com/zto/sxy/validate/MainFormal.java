...
//        initRedis();
        ...
        MainFormal instance = new MainFormal();
        instance.vertify();
    }

    private static void printRedis() {
        LOG.info("银行个数：  " + RedisUtils.getList("qf.bank").size());
        LOG.info("银行：  " + RedisUtils.getList("qf.bank").toString());
    ...
    private void vertifyEmployee() {
        ...
        String sql = "SELECT ITERFACE_ID, VALUE1 EMPLOYEE_NUMBER, VALUE7 BANK, VALUE8 BRANCH,  VALUE10 PROVINCE, VALUE11 CITY, VALUE20 DEPARTMENT, VALUE21 COMPANY FROM hscs_itf_imp_interfaces where INTERFACE_NAME = 'QF_SOA_EMPLOYEE' AND BATCH_NUM = 'ERP20180102105717'";
        try {
